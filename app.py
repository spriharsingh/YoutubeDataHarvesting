...
with col1:
    ...
    st.write(
        '(Note: This section collects data by using channel id and stores it in the :green[MongoDB] database.)')
    channel_id = st.text_input('**Enter 11 digit channel_id**')
    st.write('''Get data and store it in the MongoDB database by clicking below **:blue['Get data and store in MongoDB']**.''')
    Get_data = st.button('**Get data and store in MongoDB**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyAaPwNLdDC1p2GPuR7CThMBSKbGzklPdcc'
...
with col2:
    st.header(':green[Data Migration to MySQL]')
    st.write('''(Note: This section takes specific channel data and migrates to :blue[MySQL] database from  :green[MongoDB] database depending on your selection.
                (If option unavailable first use data collection option.)''')
    ...
    st.write('''Migrate to MySQL database from MongoDB database by clicking below **:blue['Migrate to MySQL']**.''')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        sql_host = 'localhost'
        sql_user = 'root'
        sql_password = 'GAAPS.5555'
        sql_database = 'youtube_db'
...
st.write(
    '''(Note: This section does analysis of a collection of channel data depending on your question selection and gives a table format output.)''')
...
if Check_channel:
    ...
    engine = create_engine(f'mysql+pymysql://root:GAAPS.5555@localhost/youtube_db')
...
# -----------------------------------------------------     /   Questions   /    ------------------------------------------------------------- #
st.subheader(':blue[Channels Analysis ]')
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='GAAPS.5555', db='youtube_db')
